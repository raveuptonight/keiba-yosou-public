...
class RealFeatureExtractor:
    ...
    def extract_features_for_race(
        ...
        for entry in entries:
            ...
            past_races = get_past_races(
                self.conn, kettonum, race_code, self._cache
            )
            running_styles[kettonum] = determine_running_style(past_races)

        # Pre-calculate pace prediction
        pace_info = calc_pace_prediction(
            entries, race_info, get_past_races, self.conn, self._cache
        )
    ...
    def _extract_horse_features(
        ...
        jockey_horse_stats = get_jockey_horse_combo(
            self.conn, jockey_code, kettonum, self._cache
        )
        ...
        baba_code = (
            race_info.get('shiba_babajotai_code', '1') if is_turf
        ...
        interval_stats = get_interval_stats(
            self.conn, kettonum, interval_cat, self._cache
        )
