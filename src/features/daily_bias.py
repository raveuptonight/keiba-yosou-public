...
class DailyBiasAnalyzer:
    ...
    TRACK_CONDITION = {"1": "Good", "2": "Slightly Heavy", "3": "Heavy", "4": "Bad"}
    ...
    def analyze(self, target_date: date) -> DailyBiasResult | None:
        ...
        try:
            ...
            for venue_code, data in venue_data.items():
                inner_rate = (
                    data["inner_wins"] / data["inner_total"] if data["inner_total"] > 0 else 0
                )
                outer_rate = (
                    data["outer_wins"] / data["outer_total"] if data["outer_total"] > 0 else 0
                )
                zenso_rate = (
                    data["zenso_wins"] / data["zenso_total"] if data["zenso_total"] > 0 else 0
                )
                koshi_rate = (
                    data["koshi_wins"] / data["koshi_total"] if data["koshi_total"] > 0 else 0
                )
            ...
            logger.info(
                f"Bias analysis complete: {len(venue_biases)} venues, {len(jockey_performances)} jockeys"
            )
            return result
    ...
    def load_bias(self, target_date: date) -> DailyBiasResult | None:
        ...
        try:
            ...
            venue_biases = {k: VenueBias(**v) for k, v in venue_biases_data.items()}
            jockey_performances = {k: JockeyDayPerformance(**v) for k, v in jockey_data.items()}
    ...
    def get_bias_features(
        self, bias_result: DailyBiasResult, venue_code: str, wakuban: int, kishu_code: str
    ) -> dict[str, float]:
...
def print_bias_report(bias_result: DailyBiasResult):
    ...
    for _venue_code, vb in sorted(bias_result.venue_biases.items()):
        waku_indicator = (
            "Inner favored"
            if vb.waku_bias > 0.05
            else ("Outer favored" if vb.waku_bias < -0.05 else "Neutral")
        )
        pace_indicator = (
            "Front favored"
            if vb.pace_bias > 0.05
            else ("Closer favored" if vb.pace_bias < -0.05 else "Neutral")
        )

        print(f"\n  [{vb.venue_name}] {vb.race_count}R / Track:{vb.track_condition}")
        print(
            f"    Gate: Inner{vb.inner_waku_win_rate:.1%} vs Outer{vb.outer_waku_win_rate:.1%} -> {waku_indicator}"
        )
        print(
            f"    Style: Front{vb.zenso_win_rate:.1%} vs Back{vb.koshi_win_rate:.1%} -> {pace_indicator}"
        )

    print("\n* Jockey Performance (by wins)")
    sorted_jockeys = sorted(
        bias_result.jockey_performances.values(), key=lambda x: x.wins, reverse=True
    )[:10]

    for jp in sorted_jockeys:
        print(
            f"    {jp.jockey_name}: {jp.wins}W/{jp.rides}rides "
