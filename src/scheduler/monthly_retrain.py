...
class MonthlyRetrain:
    ...
    def __init__(
        ...
        self.current_model_path = self.model_dir / "xgboost_model_latest.pkl"
    ...
    def backup_current_model(self) -> Optional[str]:
        ...
        backup_path = self.backup_dir / f"xgboost_model_{timestamp}.pkl"
    ...
    def train_new_model(self, years: int = 5) -> Dict:
        """新モデルを学習"""
        ...
        logger.info(f"新モデル学習開始（過去{years}年）")
        ...
        try:
            ...
            # 一時保存
            temp_model_path = self.model_dir / "xgboost_model_new.pkl"
            model_data = {
                'model': model,
    ...
    def run_monthly_job(self, force_deploy: bool = False, notify: bool = True):
        """月次ジョブを実行"""
        logger.info("=" * 50)
        logger.info(f"月次モデル再学習開始: {datetime.now()}")
        logger.info("=" * 50)
        ...
        # 1. 新モデル学習
        training_result = self.train_new_model(years=5)
        ...
        # 結果保存
        result_path = self.model_dir / f"retrain_result_{date.today().strftime('%Y%m')}.json"
...
def main():
    ...
    retrain = MonthlyRetrain()
    result = retrain.run_monthly_job(
