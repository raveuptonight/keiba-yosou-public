...
def build_features(
    ...
    if is_turf and sire_stats_turf:
        ...
        sire_stats = sire_stats_turf.get(
            sire_key, {"win_rate": 0.08, "place_rate": 0.25, "runs": 0}
        )
    elif sire_stats_dirt:
        sire_key = f"{sire_id}_dirt"
        sire_stats = sire_stats_dirt.get(
            sire_key, {"win_rate": 0.08, "place_rate": 0.25, "runs": 0}
        )
    else:
    ...
    features["sire_win_rate"] = sire_stats["win_rate"] * sire_confidence + BASE_WIN_RATE * (
        1 - sire_confidence
    )
    features["sire_place_rate"] = sire_stats["place_rate"] * sire_confidence + BASE_PLACE_RATE * (
        1 - sire_confidence
    )
    ...
    if sire_maiden_stats and sire_id:
        ...
        sire_m_confidence = min(
            1.0, np.log(sire_m_runs + 1) / np.log(SIRE_MAIDEN_CONFIDENCE_THRESHOLD + 1)
        )
        raw_m_win = sire_m_stats.get("win_rate", SIRE_MAIDEN_BASE_WIN)
        raw_m_place = sire_m_stats.get("place_rate", SIRE_MAIDEN_BASE_PLACE)
        features["sire_maiden_win_rate"] = raw_m_win * sire_m_confidence + SIRE_MAIDEN_BASE_WIN * (
            1 - sire_m_confidence
        )
        features["sire_maiden_place_rate"] = (
            raw_m_place * sire_m_confidence + SIRE_MAIDEN_BASE_PLACE * (1 - sire_m_confidence)
        )
    ...
    features["jockey_recent_win_rate"] = j_raw_win * j_recent_confidence + JOCKEY_BASE_WIN * (
        1 - j_recent_confidence
    )
    features["jockey_recent_place_rate"] = j_raw_place * j_recent_confidence + JOCKEY_BASE_PLACE * (
        1 - j_recent_confidence
    )
    ...
    if jockey_maiden_stats and jockey_code:
        ...
        j_maiden_confidence = min(
            1.0, np.log(j_maiden_runs + 1) / np.log(JOCKEY_MAIDEN_CONFIDENCE_THRESHOLD + 1)
        )
        j_m_raw_win = j_maiden.get("win_rate", JOCKEY_BASE_WIN)
        j_m_raw_place = j_maiden.get("place_rate", JOCKEY_BASE_PLACE)
        features["jockey_maiden_win_rate"] = j_m_raw_win * j_maiden_confidence + JOCKEY_BASE_WIN * (
            1 - j_maiden_confidence
        )
        features["jockey_maiden_place_rate"] = (
            j_m_raw_place * j_maiden_confidence + JOCKEY_BASE_PLACE * (1 - j_maiden_confidence)
        )
